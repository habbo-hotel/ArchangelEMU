...
public class NavigatorFavoriteFilter extends NavigatorFilter {
    ...
    public List<SearchResultList> getResult(Habbo habbo) {
        ...
        resultLists.add(new SearchResultList(0, "favorites", "", SearchAction.NONE, habbo.getHabboStats().getNavigatorWindowSettings().getListModeForCategory("favorites", ListMode.LIST), habbo.getHabboStats().getNavigatorWindowSettings().getDisplayModeForCategory("popular", DisplayMode.VISIBLE), rooms, true, true, DisplayOrder.ACTIVITY, -1));
        return resultLists;
    }
}